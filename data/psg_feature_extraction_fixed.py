...
Usage:
    python psg_feature_extraction_fixed.py \
...
Fixes applied vs original:
    1. load_edf        – Per-channel native fs map replaces global raw.info["sfreq"].
                         MNE unifies all channels to the max EDF rate on load; SpO2 /
                         low-rate channels were being left at 100 Hz instead of their
                         true 1 / 32 Hz, making T90, T88, ODI denominators ~100× too
                         large and T90/T88 seconds exceeding recording duration.
    2. _compute_odi    – Running-peak tracker was reset to current sample each iteration
                         so the drop condition (val < peak - threshold) was never True;
                         ODI3 and ODI4 were always 0.  Fixed to track a true running max.
    3. extract_plm_features – PLM series counter incremented once per extra valid gap
                         inside a long run instead of once per series.  Fixed with an
                         in_plm_series flag so each qualifying run contributes exactly 1.
    4. extract_sleep_architecture – TIB used last_sleep_idx as end-of-bed, excluding
                         terminal-wake epochs and producing inflated Sleep_Efficiency.
                         Fixed to use full recording length (n_epochs) as TIB end.
    5. load_edf        – Case-insensitive alias fallback only stripped whitespace; now
                         performs a genuine case-insensitive dict lookup.
...
# ─────────────────────────────────────────────────────────────────────────────
EPOCH_SEC = 30
FS_HIGH   = 256   # EEG, ECG, EMG, EOG (native)
FS_LOW    = 32    # Airflow, Thor, Abdo, Leg EMG, Position (native)
FS_SPO2   = 1     # SpO2, HR (native – keep as-is)
FS_TARGET = 100   # Downsample high-rate channels to this

STAGES = ["W", "N1", "N2", "N3", "R"]

# EDF label → internal key
...
# FIX #1 (part A): explicit per-channel native sample rates.
# MNE unifies all channels to the highest rate in the EDF on load.
# We use this map to resample each channel back to its correct native rate
# (or to FS_TARGET for high-rate channels).
CHANNEL_NATIVE_FS: Dict[str, float] = {
    "ecg":      float(FS_HIGH),
    "eog_l":    float(FS_HIGH),
    "emg_chin": float(FS_HIGH),
    "eeg1":     float(FS_HIGH),
    "flow":     float(FS_LOW),
    "thor":     float(FS_LOW),
    "abdo":     float(FS_LOW),
    "leg_emg":  float(FS_LOW),
    "spo2":     float(FS_SPO2),
    "pos":      float(FS_LOW),
}
...
def extract_sleep_architecture(ann: List[str], lights_off: float = 0.0) -> Dict:
    ...
    FIX #4: TIB now uses the full recording length (n_epochs * EPOCH_SEC) as
    the end-of-bed time instead of last_sleep_idx.  The old approach excluded
    terminal-wake epochs, making TIB too short and Sleep_Efficiency inflated.
    """
    # N4 → N3 mapping already done in STAGE_CONCEPT_MAP; this line is
    # harmless dead code but left in to preserve defensive behaviour.
    ...
    # FIX #4: use full recording end (n_epochs) as lights-on time
    time_in_bed      = n_epochs * EPOCH_SEC - lights_off
...
def extract_respiratory_features(
    ...
    """
    Compute AHI, AI, HI, CAI, OAI, REM-AHI,
    ...
    if thor is not None and abdo is not None:
        effort = np.abs(thor) + np.abs(abdo)
...
def _moving_rms(x: np.ndarray, window: int) -> np.ndarray:
    ...
    FIX #6: leading pad now uses zeros instead of rms[0] to avoid
    artificially elevated baseline at the start of the recording.
    """
    x2 = x ** 2
    cs = np.cumsum(x2)
    cs = np.concatenate([[0], cs])
    rms = np.sqrt((cs[window:] - cs[:-window]) / window)
    # FIX #6: pad with zeros, not rms[0]
    pad = np.zeros(len(x) - len(rms))
    return np.concatenate([pad, rms])


def _moving_median(x: np.ndarray, window: int) -> np.ndarray:
    """Approximate moving median using uniform_filter1d."""
...
def _detect_flow_events(
    ...
    """
    Detect segments where envelope is below thresh*baseline (and optionally
    above (1-upper_thresh)*baseline for hypopneas).
...
def _compute_odi(spo2: np.ndarray, fs: float, drop_pct: float = 3.0) -> float:
    ...
    FIX #2: Original code reset `peak = val` at the start of every non-desat
    sample, then immediately checked `val < peak - drop_pct` which is always
    False (val can't be below itself).  Fixed to maintain a running maximum
    that only updates upward, correctly detecting when SpO2 has fallen ≥drop_pct
    from the local peak.
    """
    ...
    events   = 0
    in_desat = False
    peak     = float(spo2[0])  # initialise to first sample

    for val in spo2:
        if not in_desat:
            # FIX #2: update peak upward only (running max), do not reset to val
            if val > peak:
                peak = val
            ...
            # Recovery: SpO2 climbed back to within drop_pct/2 of peak
            if val >= peak - (drop_pct / 2):
                in_desat = False
                # After recovery, reset peak to current value so the next
                # desaturation is measured from the new local baseline
                peak = val
...
def extract_cardiac_features(
    ...
    """
    SDNN, RMSSD, LF power, HF power, LF/HF ratio, mean HR per stage.
    """
    features: Dict = {}
    ...
    for st in STAGES:
        ...
        for i, stage in enumerate(ann):
            if stage != st:
                continue
            seg_rr = rr_sec[
                (rpeaks[:-1] >= i * ecg_epoch_len) &
                (rpeaks[:-1] <  (i + 1) * ecg_epoch_len)
            ]
            if len(seg_rr) > 0:
...
def extract_plm_features(
    ...
    - ≥4 consecutive qualifying movements = 1 PLM series

    FIX #3: Original counter incremented plm_count once per valid gap once
    run >= 3, so a run of N valid gaps added (N-2) series instead of 1.
    Fixed with an in_plm_series flag that ensures each qualifying run
    ...
    if len(bursts) >= 4:
        ...
        # FIX #3: use in_plm_series flag so each qualifying run counts once
        run           = 0
        in_plm_series = False
        for v in valid_series:
            if v:
                run += 1
                if run >= 3 and not in_plm_series:
                    plm_count    += 1
                    in_plm_series = True
            else:
                run           = 0
...
def extract_rem_emg_features(
    ...
    rem_epochs_n = tonic_rem_n = phasic_burst_count = 0
...
def run_batch(
    ...
    # Skip patients whose per-patient feature CSV already exists in output_dir
    edf_files_to_process = []
    for edf_path in edf_files:
        nsrrid = edf_path.stem.replace("mesa-sleep-", "")
        out_path = output_dir / f"{nsrrid}_features.csv"
        if out_path.exists():
            log.info(f"Skipping {edf_path.name}: {out_path.name} already exists.")
            continue
        edf_files_to_process.append(edf_path)

    if not edf_files_to_process:
        log.info("All EDF files already have feature CSVs; nothing to do.")
        return pd.DataFrame()
...
def main():
    ...
    parser.add_argument("--n_jobs",  type=int,  default=4,     help="Parallel workers (default: 1)")
