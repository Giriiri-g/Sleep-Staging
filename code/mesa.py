...
EDF_DIR = r"D:\MESA_Sample\edf"
XML_DIR = r"D:\MESA_Sample\annotations"
...
def main():
    ...
    for edf_path in edf_files:
        ...
        xml_path = os.path.join(XML_DIR, base + "-nsrr.xml")

        if not os.path.exists(xml_path):
            print(f"Missing XML for {edf_path}")
            continue

        stages = parse_mesa_xml(xml_path)
        rows.append([edf_path, ''.join(map(str, stages))])

    with open(OUTPUT_CSV, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["edf_path", "sleep_stages"])
